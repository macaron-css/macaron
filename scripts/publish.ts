// Largely copied over from https://github.com/TanStack/table/blob/main/scripts/publish.ts
import {
  branchConfigs,
  examplesDirs,
  latestBranch,
...
async function run() {
  ...
  const releaseType = branchConfig.prerelease
    ? 'prerelease'
    : ({ 0: 'patch', 1: 'minor', 2: 'major' } as const)[
  ...
  await Promise.all(
    examplesDirs.map(async examplesDir => {
      ...
      const pkgPath = path.resolve(examplesDir, 'package.json');
      const pkg = await readPackageJson(pkgPath);
      const exampleName = pkg.name;
      // const stat = await fsp.stat(examplesDir);
      // if (!stat.isDirectory()) continue;

      await updatePackageJson(pkgPath, async config => {
        await Promise.all(
