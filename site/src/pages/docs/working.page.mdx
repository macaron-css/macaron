...
The babel plugin then extracts all these references and the styles into a seperate file.
...
### Replacing initial content

Finally the plugin removes the code from the initial `main.tsx` file and replaces that with imports from `extracted_rxrhwu.css.ts`;
